...
const STEPS = [
  {
    icon: Search,
    title: 'Search',
    body: 'Look up colleges by name, city, course or university across the dataset.',
  },
  {
    icon: SlidersHorizontal,
    title: 'Filter & compare',
    body: 'Narrow by fees, rating, placement rate and hostel availability, then compare finalists side by side.',
  },
  {
    icon: Sparkles,
    title: 'Get matched',
    body: 'Tell us your budget, course and cutoff score - we rank colleges against your actual profile.',
  },
]
  ...
  useEffect(() => {
    collegeService
      ...
      .then((res) => {
        ...
        setTotalColleges(res.totalElements ?? 0)
        setError(false)
      })
      .catch(() => {
        setError(true)
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 py-20 md:grid-cols-[1.1fr_0.9fr] md:py-28">
          <div>
            <p className="mb-4 text-sm font-medium tracking-wide text-gold">
              {totalColleges
                ? `${totalColleges} real Indian colleges, and counting`
          ...
          <div className="relative hidden md:block">
            <div className="rounded-3xl border border-line bg-white p-6 shadow-[0_20px_60px_-20px_rgba(20,33,61,0.25)]">
              <p className="font-display text-sm font-semibold text-ink">
                Why students trust the data
              </p>

              <ul className="mt-4 space-y-3 text-sm text-muted">
                <li className="flex items-center gap-2">
                  <ScanSearch className="h-4 w-4 text-teal" />
                  No invented colleges or fabricated stats
                </li>

                <li className="flex items-center gap-2">
                  <ScanSearch className="h-4 w-4 text-teal" />
                  Fees, cutoffs and placement rates from real records
                </li>

                <li className="flex items-center gap-2">
                  <ScanSearch className="h-4 w-4 text-teal" />
                  Transparent, criteria-based recommendation scoring
                </li>
              </ul>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="mt-8 rounded-2xl border border-line bg-white p-8 text-center">
              <p className="font-medium text-ink">
                College data is taking a little longer to load.
              </p>

              <p className="mt-2 text-sm text-muted">
                The backend may be starting up. Please refresh the page in a
                few seconds.
              </p>
      ...
      <section className="mx-auto max-w-6xl px-5 py-20">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          <div className="rounded-2xl border border-line bg-white p-6">
            ...
            <p className="mt-2 text-sm text-muted">
              Every college, fee figure and placement number comes from the
              underlying dataset. Our AI assistant is instructed to only answer
              from that data, and says so plainly when something isn't
              available.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-line bg-white p-6">
            ...
            <p className="mt-2 text-sm text-muted">
              Recommendations weigh course fit, location, budget, cutoff
              compatibility, placement rate, rating and hostel needs - so the
              ranking reflects what actually matters to you.
            </p>
          </div>
        </div>
      </section>
